...
class DataLoader:
    ...
    def preprocess_input(self, sentence, parse_tree, tokenizer):
        ...
        if synt_len<self.synt_len:
            for index in range(synt_len, self.synt_len):
                synt_seq.append(0)
        else:
            synt_seq = synt_seq[:self.synt_len]

        if lex_len<self.synt_len:
            for index in range(lex_len, self.synt_len):
                lex_seq.append(0)
        else:
            lex_seq = lex_seq[:self.synt_len]
...
if __name__=="__main__":
    ...
    lex_len, synt_len = 40, 40
    embedding = "fasttext"
    file_path = "../Datasets/pandata/easy/"
    # file_path = "./sub_data"

    dl = DataLoader(file_path=file_path, lex_len=lex_len, synt_len = synt_len, embedding=embedding, parse=False)

    print('pickling...')
    with open("lambda_data_parsetrees_%s_len_%s.p" %(embedding,lex_len), "wb") as fid:
