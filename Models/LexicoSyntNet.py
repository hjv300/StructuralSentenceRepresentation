...
class LexicoSyntNet:
    ...
    def create_base_network_self_attention(self, vocab_size, embed_dim, rep_dim, input_length):
        ...
        sentence_encoder = Bidirectional(LSTM(units=256, activation='tanh', return_sequences=True))(word_embeddings)
    ...
    def create_network(self):
        ...
        model.summary()
        return lexical_subnetwork, syntactic_subnetwork, model

    def train_network(self, data, epochs=50, batch_size=400):

        optimizer = keras.optimizers.Nadam(0.0001)
        checkpoint = ModelCheckpoint(filepath="checkpoints/%s_flatt_40_80-epoch{epoch:02d}.hdf5" %(self.base_network), verbose=1, save_weights_only=True)
    ...
    def load_network(self, model_file):
        ...
        print("number of layers: ",len(self.lexicosyntnet.layers))
